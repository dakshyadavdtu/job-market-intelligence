#!/usr/bin/env python3
"""Deploy jmi_analytics_v2.v2_eu_sankey_location_to_company_monthly."""
...
def main() -> int:
    path = ROOT / "infra" / "aws" / "athena" / "analytics_v2_eu_sankey_helper.sql"
    ...
    for line in raw.splitlines():
        if lines or line.strip().startswith("CREATE"):
            lines.append(line)
    stmt = "\n".join(lines).strip()
    print("Deploying v2_eu_sankey_location_to_company_monthly...", file=sys.stderr)
    qid = run_sql(stmt)
    wait(qid)
