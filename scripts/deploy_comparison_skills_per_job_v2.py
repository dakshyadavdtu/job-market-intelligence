#!/usr/bin/env python3
"""Deploy jmi_analytics_v2.v2_cmp_skills_per_job_april_2026 (EU vs IN skills-per-job for April 2026)."""
...
def main() -> int:
    path = ROOT / "infra" / "aws" / "athena" / "analytics_v2_comparison_skills_per_job.sql"
    ...
    for line in raw.splitlines():
        if lines or line.strip().startswith("CREATE"):
            lines.append(line)
    stmt = "\n".join(lines).strip()
    print("Deploying v2_cmp_skills_per_job_april_2026...", file=sys.stderr)
    qid = run_sql(stmt)
    wait(qid)
