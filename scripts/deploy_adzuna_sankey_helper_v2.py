#!/usr/bin/env python3
"""Deploy jmi_analytics_v2.v2_in_sankey_state_to_company_monthly."""
...
def main() -> int:
    path = ROOT / "infra" / "aws" / "athena" / "analytics_v2_adzuna_sankey_helper.sql"
    ...
    for line in raw.splitlines():
        if lines or line.strip().startswith("CREATE"):
            lines.append(line)
    stmt = "\n".join(lines).strip()
    print("Deploying Sankey view...", file=sys.stderr)
    qid = run_sql(stmt)
    wait(qid)
