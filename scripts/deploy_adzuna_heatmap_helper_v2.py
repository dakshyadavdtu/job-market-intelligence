#!/usr/bin/env python3
"""Deploy jmi_analytics_v2.v2_in_heatmap_state_skill_monthly."""
...
def main() -> int:
    path = ROOT / "infra" / "aws" / "athena" / "analytics_v2_adzuna_heatmap_helper.sql"
    ...
    for line in raw.splitlines():
        if lines or line.strip().startswith("CREATE"):
            lines.append(line)
    stmt = "\n".join(lines).strip()
    print("Deploying heatmap view...", file=sys.stderr)
    qid = run_sql(stmt)
    wait(qid)
