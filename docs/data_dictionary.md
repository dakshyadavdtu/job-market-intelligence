...
**Multi-source:** Fact tables include `source` (e.g. `adzuna_in`). Run Gold with `--source adzuna_in` to build partitions from that source’s merged Silver. Each Gold run updates **that source’s** pointer Parquet only: **`gold/source=arbeitnow/latest_run_metadata/`** for default/EU runs and **`gold/source=adzuna_in/latest_run_metadata/`** for Adzuna runs (no cross-source overwrite). In **`jmi_gold_v2`**, the EU pointer table is **`latest_run_metadata_arbeitnow`** (Glue **LOCATION** under the Arbeitnow prefix); Adzuna uses **`latest_run_metadata_adzuna`** (see `ddl_gold_latest_run_metadata_*.sql`). Legacy **`jmi_gold.latest_run_metadata`** (v1) may still exist separately.
