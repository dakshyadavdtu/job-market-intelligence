...
-- Projection month bounds MUST match ddl_gold_* TBLPROPERTIES projection.ingest_month.range.
-- =============================================================================
...
-- 2) Base Gold: row counts for latest run_id + ingest_month within projection range
...
WHERE run_id = (SELECT run_id FROM jmi_analytics.latest_pipeline_run)
  AND ingest_month BETWEEN '2018-01' AND '2035-12';
...
SELECT 'sample_pipeline_run_summary_latest' AS check_name,
       source, ingest_month, run_id, status, skill_row_count, role_row_count
FROM jmi_analytics.pipeline_run_summary_latest
LIMIT 3;
