...
-- -----------------------------------------------------------------------------
-- v2_strict_common_* (from comparison_* + latest_run_metadata)
-- -----------------------------------------------------------------------------
...
WITH lr_an AS (SELECT run_id FROM jmi_gold_v2.latest_run_metadata LIMIT 1),
     ...
     pol AS (SELECT * FROM jmi_analytics_v2.comparison_time_window_policy LIMIT 1)
SELECT
  CAST('strict_common_month' AS VARCHAR) AS layer_scope,
...
WITH lr_an AS (SELECT run_id FROM jmi_gold_v2.latest_run_metadata LIMIT 1),
     lr_ad AS (SELECT run_id FROM jmi_gold_v2.latest_run_metadata_adzuna LIMIT 1),
     inter AS (SELECT posted_month FROM jmi_analytics_v2.comparison_strict_intersection_months),
     agg AS (
       SELECT
