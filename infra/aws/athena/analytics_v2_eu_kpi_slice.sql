...
-- Replaces missing sheet1_kpis for v2: Gold v2 + concentration metrics + Silver remote share.
-- Latest EU run: jmi_gold_v2.latest_run_metadata.
-- top5_skill_tag_share = sum(job_count) of top 5 skill rows / sum(all skill job_count) for that month
--   (tag-weight concentration; NOT share of unique jobs).
...
SELECT
  ...
  CASE
    WHEN tt.tag_sum_total > 0 AND k5.top5_skill_tag_jobs IS NOT NULL
      THEN CAST(k5.top5_skill_tag_jobs AS double) / CAST(tt.tag_sum_total AS double)
    ELSE NULL
...
LEFT JOIN skill_tag_totals tt ON r.posted_month = tt.posted_month AND r.run_id = tt.run_id
